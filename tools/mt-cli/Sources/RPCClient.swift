...
struct RPCClient {
    ...
    /// `POST /v1/record` blocks until the start settles — mic gate, engine
    /// sync, queue rebuild, loop construction — so the "rest of the API is
    /// fast" premise behind `requestTimeoutSeconds` does not hold for it. Set
    /// above the server's own 20 s join bound so the server always answers
    /// first: a client-side timeout would report failure for a start that
    /// then succeeds anyway, since the work continues after the caller
    /// disconnects.
