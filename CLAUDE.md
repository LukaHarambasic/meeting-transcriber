...
- `AppState`'s `PermissionsController` runs the check at launch (`.task { await appState.permissions.check() }`) and re-runs it, debounced, on app activation. `WatchLoop` separately gates each manual-recording start against the same check, scoped to what that recording actually opens (an app/system start needs Screen Recording as the tap's preflightable proxy; a microphone-only start needs only the mic grant).
