...
**View architecture:**
- `SettingsView` receives its dependencies as stored properties (not `@State`): the engine instances, `recognitionStatsLog`, an `enrollmentDiarizerFactory`, the `namingDialogActive`/`pipelineBusy` state flags, and an `onSpeakerMutate` callback.
...
- `NSOpenPanel` type lists come from `AudioImportTypes`, not inline literals — the panels are manual-QA-only, so the pure type list is what tests can pin. Only MKV/WebM are ffmpeg-gated; AMR, 3GPP and Ogg (`.opus`/`.ogg`) decode natively and must stay out of `FFmpegHelper.ffmpegOnlyExtensions`, whose members skip `AVAudioFile`/`AVAsset` entirely.
- **The GUI file-import picker is gone** (the menu's "Process Audio/Video Files…", ⌘P): it read as "process the recordings you already made", which is not what it did, and recordings reach the pipeline automatically the moment they stop. File import survives only via the automation API (`POST /v1/jobs`, `POST /v1/transcribe`) through `PipelineController.enqueueFiles`, which is still the single entry point. Consequence: `PairedImportPanelDelegate` and `AudioImportTypes.pickerTypes` now have test callers only — delete both if no GUI picker returns.
...
- Every recording is user-initiated: the menu bar's single **Record** (⌘R) item taps the whole
  system output mixdown plus the microphone (`RecordingSource.forSystem`, honouring "No
  Microphone" by dropping the mic channel); the narrower mic-only and single-app shapes exist
  only behind `POST /v1/record` (see Debug RPC server below). `WatchLoop` keeps its name but is a
  manual-recording session driver, not a poll loop — `WatchLoop.State` is `idle`/`recording`/`error`.
...
- **Never route automatic/placeholder speaker names through the CONFIRM path.** `completeSpeakerNaming` reaches `SpeakerMatcher.updateDB`, which folds the embedding into a running-mean centroid with no history — auto-confirming "Speaker 1" would merge every unrelated voice from every meeting under one permanent identity. Auto-naming must use the SKIP path (which learns nothing) plus display-only relabelling, and must leave the internal `M_`/`R_` prefixes intact (`EchoEmbeddingQuarantine` parses them).
...
- `AppSettings.protocolLanguage` string (default `"English"` in this personal variant — upstream defaults to `"German"`) is substituted into the prompt as `{LANGUAGE}`. `{MEETING_DATE}` (`YYYY-MM-DD`) and `{MEETING_TIME}` (`HH:mm`) resolve from a captured recording start time, or to `Unknown` for imports and recovery jobs. Only a captured recording start adds the authoritative meeting-metadata preamble, so processing time is never presented as a meeting time.
...
**UI:**
- `MenuBarIcon` renders an animated waveform reflecting pipeline state (idle, recording, transcribing, diarizing, protocol). Recording is a sine wave spread across the five bars and advanced one frame per tick (`recordingBarHeight(bar:frame:)`), not the six hand-written random height rows it replaced — those read as jitter at 18pt.
- **Colour in the icon means exactly one thing: an error.** Every state renders as a *template* image so AppKit inverts it for light/dark and for the open-menu highlight; the single exception is `image(..., errorOverlay: true)`, which composites a red dot and forces non-template rendering. Three earlier red marks (record-only dot, permission exclamation, per-channel half-tinted bars) were removed to get there: each forced a non-template image, so the icon stayed black-on-blue under the menu highlight, and none was explained anywhere in the UI. The badge body keeps animating under the dot, so a recording that hits a problem does not freeze into something that reads as "stopped".
- `RecordingIssue.compose(permissionProblems:recordingError:micSilent:appSilent:)` is the pure function returning **at most one** issue (headline, detail, optional `Remedy` that opens a System Settings pane). It drives both the dot (`AppState.hasIssue`) and the menu row (`AppState.currentIssue`), so the icon can never show a problem the menu declines to name. Precedence is by what blocks a recording from *starting*: permission problem → `WatchLoop.lastError` → silent capture channel. `lastError` is cleared when the next recording opens capture, so a stale failure never outlives the recording that fixed it.
- `MenuBarView` takes `issue` as its own property, separately from `status`. Load-bearing: `AppState.currentStatus` is nil whenever nothing is recording, so the previous row (which read `status?.error` *and* required `state == .error`) was unreachable for exactly the failures that prevent a recording from starting.
- **No update checking.** This fork has diverged from upstream, so there is nothing to stay current with: there is no `UpdateChecker`, no Updates section in Settings, no "Update Available" menu row, and no `updateAvailable` badge. A new version arrives through the Homebrew Cask like any other formula.

**Recording durability** (the "it stops when the Mac sleeps and nothing is saved" cluster) — four mechanisms, none sufficient alone; full rationale in `docs/architecture-macos.md` → Recording Durability:
- `RecordingPowerAssertion` holds `kIOPMAssertPreventUserIdleSystemSleep` for the length of a recording. **System sleep, deliberately not display sleep:** idle system sleep suspends the process and tears down the audio devices, which ends capture; display sleep does not. So the screen still darkens and still locks. `WatchLoop` takes it after `recorder.start()` succeeds (a refused start must not leak one) and releases it on both stop paths.
- An assertion only blocks *idle* sleep. A closed lid, Apple menu → Sleep and a flat battery sleep the Mac anyway, so `PowerEventMonitor` (`NSWorkspace.shared.notificationCenter`, **not** `NotificationCenter.default` — the same names there are never posted) routes will-sleep to `WatchingController.finalizeForSleep()`, which stops and enqueues through the same path the menu's Stop item uses.
- A `recorder.stop()` that throws leaves the per-track WAVs and the in-progress marker on disk. `WatchLoop.salvageInterruptedRecording()` re-mixes them immediately (`recoverCrashedRecordings(minAge: 0)`, then `recoverOrphanedRecordings()`) instead of waiting for a launch that, for a menu-bar app that never quits, may be weeks away.
- `WatchingController.recoverAfterWake()` runs the same recovery on did-wake for interruptions that gave no notice. **Guarded on nothing recording:** `minAge: 0` against a live recording would re-mix tracks still being written. That guard is hit in normal operation — when the assertion works and only the display slept, the recording is still running at did-wake.
- Both recovery calls and the sleep blocker are injected (`salvageInterrupted`, `recoverInterrupted`, `makeSleepBlocker`) because the production implementations touch IOKit and `AppPaths.recordingsDir`; the test factory in `XCTestCase+WatchingController.swift` defaults them *away* from production so no test can reach either by omission.

**Still-recording confirmation:**
- `RecordingConfirmationPolicy` (pure) returns `.wait | .prompt | .stopUnconfirmed` from `now`, `confirmedAt` and `promptedAt`. Defaults: 30-minute interval, 5-minute grace. Driven by `WatchLoop.stepConfirmation(now:)` from the manual-recording monitor's poll, *after* the hard stop conditions so a recording whose target exited is attributed to that instead.
- A non-nil `promptedAt` suspends the interval entirely — one ask outstanding at a time, so an ignored ask gets the grace period rather than a second ask and a moving deadline.
- Posted `.timeSensitive` under the `recording-confirmation` category with a **Keep Recording** action. Time-sensitive is required, not polish: the ask has to reach someone in a meeting, which is when Focus is on, and an ask nobody can see is one nobody can answer — the grace period then stops their recording.
- **`.timeSensitive` does not survive a dev build, and this feature's correctness depends on it.** `run_app.sh` signs without `com.apple.developer.usernotifications.time-sensitive` (no provisioning profile for `app.meetingtranscriber.dev`; it says so on every build — see issue #543). So in a dev build the ask is Focus-suppressible, and the path that follows a suppressed ask is *stop the recording after the grace period*: with Focus on during a meeting, a dev build loses the meeting's tail. General rule for this repo: any feature whose correctness rests on a notification actually being *delivered* has a dev/release behaviour split, and a dev-build manual QA pass will not reproduce the shipped behaviour. Before treating an auto-stop as a bug, check which build it happened in.
- Which notification carries the action is decided by `NotificationManager.categoryID(forTitle:)` matching `RecordingConfirmationPolicy.promptTitle`, rather than by adding a parameter to `AppNotifying.notify` (that protocol is witnessed by a silent no-op and by test doubles; its own doc comment explains why a new argument invites conformers that drop it). Consequence: the title is load-bearing, and the policy owns it because `NotificationManager` is not `@MainActor`.
- Both the action button and a body click (`UNNotificationDefaultActionIdentifier`) count as an answer; an explicit dismiss does not. An unanswered ask **stops and saves** — the audio is enqueued, so a mistimed stop costs the rest of the meeting, never what was already captured.

**Permission health check:**
- `PermissionHealthCheck` verifies each TCC permission by combining the system verdict with a live probe. Each resolves to `PermissionStatus` (`.healthy | .denied | .broken | .notDetermined`). `.broken` means TCC says allowed but the probe disagrees — fix is to toggle the permission off and on in System Settings. Only Microphone and Screen Recording are checked (Accessibility was removed with meeting auto-detection — its only consumer was the Teams participant/mute reader).
- **Two entry points, and which one a caller gets is load-bearing.** `PermissionHealthCheck.runPassive()` reads TCC only and opens no audio device; `runForRecordingStart(source:)` additionally probes the microphone, but only when `source.capturesMicrophone`. Opening an input is not a read-only act: on a Bluetooth headset it drags the device out of A2DP into HFP and back, which audibly wrecks the user's playback and usually needs a manual reconnect, and a headset is normally the default *input* as well as the default output. The app used to run the ~150 ms `AVAudioEngine` probe at launch and on every `didBecomeActive`, and opening the menu bar dropdown activates the app, so a user's music broke every time they looked at the menu. Debouncing is not a fix and must not be reached for again: it changes how often the device is opened, not whether it is, and once is enough. Accepted cost: `.broken` for the microphone is now reachable only from the recording gate, so it surfaces when a mic recording starts rather than proactively. `Tests/MicProbeContainmentTests.swift` pins the caller split by scanning `Sources/`; a new proactive caller of `checkMicrophoneLive`/`probeMicrophone` fails it.
- `AppState`'s `PermissionsController` runs the passive check at launch (`.task { await appState.permissions.check() }`) and re-runs it, debounced, on app activation; its `probe` seam must stay `runPassive`. `WatchLoop` separately gates each manual-recording start via its own `permissionChecker`, which takes the `RecordingSource` and so is scoped to what that recording actually opens (an app/system start needs Screen Recording as the tap's preflightable proxy; a microphone-only start needs only the mic grant). `WatchingController` no longer hands the loop `PermissionsController.health`: that cache is passive and can never report `.broken`, so reusing it would leave nothing anywhere running the probe. The gate is where the probe is free, because the recorder opens the same device milliseconds later.
- When unhealthy: `AppState.currentIssue` resolves to a `RecordingIssue`, which puts a row in the menu naming the permission, what its absence costs, and a button opening the pane that fixes it; `AppState.hasIssue` makes `MenuBarIcon` composite the red error dot; `BadgeKind.compute()` returns `.error` when idle with a problem (so `/state.badge` reports it); and a deduped notification is posted via `NotificationManager`. The menu row is the load-bearing one: a denied Screen Recording grant refuses every recording that taps a process, and before it the whole report of that refusal was a banner that had auto-dismissed by the time the user opened the menu, which said "Idle".
...
- `trigger` (`auto` | `manual`, added in version 2) says which call site produced the recording, so a fleet consumer can discard a very short *auto* capture as a false trigger while always processing an equally short *manual* one — duration alone cannot tell those apart. Every recording written today is `manual`; `auto` is kept only for schema compatibility with sidecars written before meeting auto-detection was removed. Every field added after version 1 must decode as optional, because `RecordingSidecar.read` is a `try?` and one unrecognised value would otherwise discard the *entire* sidecar on the reimport path.
...
- Menu bar: **not marked at all.** The mode used to paint a persistent red dot on the icon; it was removed because the icon's colour is now reserved for "something is wrong" (see UI below), and a permanent dot for a deliberate user choice competed with the one mark that means a problem. Settings tabs dim Transcription / Protocol / VAD / Diarization sections via `View.recordOnlyDisabled(_:)` and show a banner in the General tab pointing at the active output dir.
...
1. **Pure logic first.** Extract decision logic into a value type
   (`BadgeKind.compute`, `LiveCaptionsGate`, `ManualRecordingMonitorPolicy` pattern) and put
...
Two complementary E2E approaches (fixture-based xctest `e2e.yml` and live-recording
`e2e-app.yml`/`scripts/e2e-app.sh`), the CI trigger labels (`run-e2e`/`run-quality`), when
...
Read it before touching the E2E workflows, `scripts/e2e-app.sh`,
